...
function sheetHTML(lang){
  ...
  const hasPh = S.img.dish || S.img.table;
  ...
  '<div class="inner">'+
    ...
    '<div class="s-title"><div class="s-titleL">'+
...
function autofit(node){
  ...
  for(let i=0;i<steps.length;i++){
    ...
    let ok=true;
    for(const c of list){ if(c.scrollHeight > c.clientHeight+1){ ok=false; break; } }
    if(ok) break;
  }
  return fit;
}
function renderSheet(){
  ...
  if(fit<=0.62){
    w.innerHTML='<div class="warn">Le texte a été réduit à '+Math.round(fit*100)+
      ' % pour tenir sur la page. Scindez la recette en deux fiches pour rester lisible en cuisine.</div>';
  } else w.innerHTML='';
  fitPreview();
}
function fitPreview(){
